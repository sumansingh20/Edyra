...
function createDocumentationPDF() {
  ...
  doc.moveDown(5);
  doc.font('Helvetica-Bold').fontSize(40).fillColor('#FFFFFF').text('EDYRA', { align: 'center' });
  doc.moveDown(0.5);
  doc.font('Helvetica').fontSize(16).fillColor('#10B981').text('ENTERPRISE ACADEMIC ECOSYSTEM', { align: 'center', letterSpacing: 3 });
  
  doc.moveDown(4);
  doc.font('Helvetica-Bold').fontSize(26).fillColor('#FFFFFF').text('Comprehensive Institutional &\nTechnical Whitepaper', { align: 'center', lineGap: 12 });

  doc.moveDown(6);
  doc.rect(doc.page.width / 2 - 150, doc.y, 300, 1.5).fillColor(primaryColor).fill();
  doc.moveDown(2);
  ...
  doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(24).text('Table of Contents');
  doc.moveDown(0.5);
  doc.rect(doc.x, doc.y, 80, 3).fill(primaryColor);
  doc.moveDown(2);

  const tocItems = [
    { title: '1. Executive Summary & Enterprise Vision', desc: 'Platform overview, institutional target demographics, and core value proposition.', page: '3' },
    { title: '2. High-Performance Cloud Architecture', desc: 'Decoupled Next.js 14, Express microservices, Mongoose .lean(), and Socket.IO telemetry.', page: '3' },
    { title: '3. Safe Exam Browser & Proctoring Engine', desc: 'Fullscreen enforcement, JS event interception, blur/focus auditing, and fingerprinting.', page: '4' },
    { title: '4. Comprehensive Role-Based Workflows', desc: 'Granular breakdown of Student, Teacher, Administrator, ERP, and AI collaboration portals.', page: '5' },
    { title: '5. Database Schema & Entity Relationships', desc: 'Detailed specifications for all 9 Mongoose collections, fields, and indexing strategies.', page: '6' },
    { title: '6. Exhaustive API Route Directory', desc: 'Complete REST endpoint mapping across Auth, Admin, Teacher, Student, and Exam Engine.', page: '7' },
    { title: '7. Production Deployment & Vercel Config', desc: 'Step-by-step Vercel Serverless cloud deployment guides and environment variable mapping.', page: '8' },
    { title: '8. Verification Credentials & Licensing', desc: 'Pre-seeded production demo accounts, login instructions, and MIT License declaration.', page: '8' }
  ];

  tocItems.forEach(item => {
    doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(12).text(item.title, { continued: true });
    ...
    doc.moveDown(0.3);
    doc.font('Helvetica').fontSize(10).fillColor(greyText).text(item.desc, { width: doc.page.width - 120 });
    doc.moveDown(1.2);
  });
  ...
  const addSectionHeader = (title) => {
    doc.moveDown(1.5);
    doc.rect(doc.x, doc.y, doc.page.width - 100, 28).fill(lightBg);
    doc.rect(doc.x, doc.y, 6, 28).fill(primaryColor);
    doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(15).text(title, doc.x + 16, doc.y + 7);
    doc.moveDown(1.5);
  };
  ...
  const addSubHeader = (title) => {
    doc.moveDown(1);
    doc.fillColor(primaryColor).font('Helvetica-Bold').fontSize(13).text(title);
    doc.moveDown(0.5);
  };

  // Helper for Paragraphs
  const addParagraph = (text) => {
    doc.fillColor(greyText).font('Helvetica').fontSize(10.5).text(text, { lineGap: 6, align: 'justify' });
    doc.moveDown(0.8);
  };

  // Helper for Bullet Points
  const addBullet = (title, desc) => {
    doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(10.5).text('•  ' + title + ': ', { continued: true })
  ...
  const addCallout = (title, text, type = 'info') => {
    doc.moveDown(0.5);
    const boxColor = type === 'warning' ? '#FEF3C7' : '#E0F2FE';
    const borderColor = type === 'warning' ? '#F59E0B' : secondaryColor;
    const titleColor = type === 'warning' ? '#92400E' : '#0369A1';
    
    const startY = doc.y;
    doc.rect(doc.x, startY, doc.page.width - 100, 65).fill(boxColor);
    doc.rect(doc.x, startY, 4, 65).fill(borderColor);
    
    doc.fillColor(titleColor).font('Helvetica-Bold').fontSize(11).text(title, doc.x + 15, startY + 10);
    doc.fillColor(greyText).font('Helvetica').fontSize(10).text(text, doc.x + 15, startY + 28, { width: doc.page.width - 130, lineGap: 4 });
    doc.moveDown(2);
  };
  ...
  const addTableRow = (col1, col2, col3, isHeader = false, isAlt = false) => {
    ...
    if (isHeader) {
      doc.rect(doc.x, currentY, doc.page.width - 100, rowHeight).fill(darkColor);
      doc.fillColor('#FFFFFF').font('Helvetica-Bold').fontSize(10);
    } else {
      doc.rect(doc.x, currentY, doc.page.width - 100, rowHeight).fill(isAlt ? lightBg : '#FFFFFF');
      doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(9.5);
    }

    doc.text(col1, doc.x + 15, currentY + (isHeader ? 7 : 8), { width: 130 });
    doc.font(isHeader ? 'Helvetica-Bold' : 'Helvetica').fillColor(isHeader ? '#FFFFFF' : greyText);
    doc.text(col2, doc.x + 150, currentY + (isHeader ? 7 : 8), { width: 140 });
    doc.font('Helvetica-Bold').fillColor(isHeader ? '#FFFFFF' : primaryColor);
    doc.text(col3, doc.x + 300, currentY + (isHeader ? 7 : 8), { width: doc.page.width - 415 });
  ...
  doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(22).text('1. Executive Summary & Enterprise Vision');
  doc.moveDown(0.5);
  doc.rect(doc.x, doc.y, 60, 3).fill(primaryColor);
  doc.moveDown(1.5);
  ...
  doc.moveDown(2);
  doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(22).text('2. High-Performance Cloud Architecture');
  doc.moveDown(0.5);
  doc.rect(doc.x, doc.y, 60, 3).fill(primaryColor);
  doc.moveDown(1.5);
  ...
  doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(22).text('3. Safe Exam Browser & Proctoring Engine');
  doc.moveDown(0.5);
  doc.rect(doc.x, doc.y, 60, 3).fill(primaryColor);
  doc.moveDown(1.5);
  ...
  doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(22).text('4. Comprehensive Role-Based Workflows');
  doc.moveDown(0.5);
  doc.rect(doc.x, doc.y, 60, 3).fill(primaryColor);
  doc.moveDown(1.5);
  ...
  doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(22).text('5. Database Schema & Entity Relationships');
  doc.moveDown(0.5);
  doc.rect(doc.x, doc.y, 60, 3).fill(primaryColor);
  doc.moveDown(1.5);
  ...
  doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(22).text('6. Exhaustive API Route Directory');
  doc.moveDown(0.5);
  doc.rect(doc.x, doc.y, 60, 3).fill(primaryColor);
  doc.moveDown(1.5);
  ...
  doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(22).text('7. Production Deployment & Vercel Config');
  doc.moveDown(0.5);
  doc.rect(doc.x, doc.y, 60, 3).fill(primaryColor);
  doc.moveDown(1.5);
  ...
  doc.moveDown(2);
  doc.fillColor(darkColor).font('Helvetica-Bold').fontSize(22).text('8. Verification Credentials & Licensing');
  doc.moveDown(0.5);
  doc.rect(doc.x, doc.y, 60, 3).fill(primaryColor);
  doc.moveDown(1.5);
  ...
  doc.moveDown(4);
  doc.rect(doc.x, doc.y, doc.page.width - 100, 1).fillColor('#E5E7EB').fill();
  doc.moveDown(1.5);
  ...
  // Add Page Numbers & Running Headers across all pages
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    if (i > 0) { // Skip cover page
      ...
      doc.text('Edyra Enterprise Academic Ecosystem — Comprehensive Technical Whitepaper', 50, 25);
      doc.rect(50, 35, doc.page.width - 100, 0.5).fillColor('#E5E7EB').fill();

      // Running Footer
      doc.font('Helvetica').fontSize(9).fillColor('#9CA3AF');
      doc.text(`Page ${i + 1} of ${range.count}`, 50, doc.page.height - 35, { align: 'center' });
    }
  }
