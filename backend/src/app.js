...
const setCorsHeaders = (req, res) => {
  ...
  // Allow configured origins OR local development
...
const initializeDB = async () => {
  ...
  try {
    ...
    // AUTO-SEED VERCEL DEPLOYMENTS
    const adminCount = await User.countDocuments({ role: 'admin' });
    if (adminCount === 0) {
      console.log('[DB] No admin found. Running auto-seed for first-time Vercel deployment...');
      try {
        const seedUsers = [
          {
            email: 'admin@edyra.com', password: 'Suman@123', firstName: 'Super', lastName: 'Admin',
            role: 'admin', isActive: true, isVerified: true
          },
          {
            email: 'teacher@edyra.com', password: 'Suman@123', firstName: 'John', lastName: 'Smith',
            role: 'teacher', employeeId: 'T001', department: 'Computer Science', isActive: true, isVerified: true
          },
          {
            email: 'student@edyra.com', password: 'Suman@123', firstName: 'Jane', lastName: 'Doe',
            role: 'student', studentId: 'STU001', rollNumber: 'CS2024001', department: 'Computer Science',
            batch: '2024', isActive: true, isVerified: true
          }
        ];
        ...
        console.log('[DB] Auto-seed complete. Default credentials ready.');
      } catch (seedErr) {
...
app.use(async (req, res, next) => {
  // Skip DB ONLY for health check and root endpoints
  if (
    req.path === '/' ||
    req.path === '/api/health' ||
    req.path === '/favicon.ico' ||
    req.path.startsWith('/api/auth/server-time')
  ) {
    return next();
  }
