...
export const examSessionController = {
  ...
  async getExamSession(req, res, next) {
    try {
      ...
      const existingSession = await ExamSession.findOne({ sessionToken });

      if (!existingSession) {
        throw new AppError('Session not found', 404);
      }

      if (['submitted', 'force_submitted', 'expired', 'violation_terminated', 'terminated'].includes(existingSession.status)) {
        return res.json({
          success: true,
          ...
          totalViolations: existingSession.violationCount,
        });
      }
  ...
  async submitExam(req, res, next) {
    try {
      ...
      if (!validation.valid && validation.reason === 'session_expired') {
        const expiredSession = await ExamSession.findOne({ sessionToken });
        return res.json({
          success: true,
          message: 'Exam auto-submitted because time expired',
          status: expiredSession?.status || 'expired',
          submittedAt: expiredSession?.submittedAt,
          answeredQuestions: expiredSession?.answers?.length || 0,
          totalViolations: expiredSession?.violationCount || 0,
        });
      }
      ...
      try {
        const questions = await Question.find({ exam: session.exam, isActive: true }).lean();
          ...
          if (!existingSubmission) {
            ...
            for (const ans of session.answers) {
              ...
              const answerEntry = {
                ...
                numericalAnswer: q.questionType === 'numerical'
                  && ans.textAnswer !== undefined
                  && ans.textAnswer !== null
                  && !Number.isNaN(Number(ans.textAnswer))
                  ? Number(ans.textAnswer)
                  : null,
              };

              // Convert selectedOption index to option ObjectId for MCQ types
              if (q.questionType === 'mcq-single' || q.questionType === 'mcq-multiple' || q.questionType === 'true-false' || q.questionType === 'image-based') {
                const selectedIndices = Array.isArray(ans.selectedOptions) && ans.selectedOptions.length > 0
                  ? ans.selectedOptions
                  : (ans.selectedOption !== undefined && ans.selectedOption !== null ? [ans.selectedOption] : []);

                answerEntry.selectedOptions = selectedIndices
                  .map(selected => {
                    if (typeof selected === 'number') return q.options[selected]?._id;
                    const selectedString = selected?.toString();
                    return q.options.find(option => option._id.toString() === selectedString)?._id;
                  })
                  .filter(Boolean);
              }
...
export const batchController = {
    ...
    try {
      ...
      if (activeBatch) {
        ...
        for (const session of activeSessions) {
          const linkedSubmission = await findSubmissionForSession(session);
          if (linkedSubmission && linkedSubmission.status === 'in-progress') {
            await autoSubmitExam(linkedSubmission, 'admin-force', {
              reason: 'Batch ended',
              ipAddress: req.ip,
              userAgent: req.headers['user-agent'],
              totalViolations: session.violationCount || 0,
            });
          } else {
            await session.forceSubmit('batch_ended_force_submit', req.ip);
          }
        }
  ...
  async getBatchStatus(req, res, next) {
    try {
      ...
      const batches = await ExamBatch.find({ exam: examId }).sort({ batchNumber: 1 }).lean();
      ...
      const batchStats = await Promise.all(batches.map(async (batch) => {
        ...
        const submittedCount = await ExamSession.countDocuments({
          exam: examId,
          batch: batch.batchNumber,
          status: { $in: ['submitted', 'force_submitted', 'expired', 'violation_terminated', 'terminated'] },
        });
  ...
  async lockBatch(req, res, next) {
    try {
      ...
      for (const session of activeSessions) {
        ...
        if (linkedSubmission && linkedSubmission.status === 'in-progress') {
          await autoSubmitExam(linkedSubmission, 'admin-force', {
            reason: `Batch ${batchNumber} locked`,
            ipAddress: req.ip,
            userAgent: req.headers['user-agent'],
            totalViolations: session.violationCount || 0,
          });
        } else {
          await session.forceSubmit('batch_locked', req.ip);
        }
      }
