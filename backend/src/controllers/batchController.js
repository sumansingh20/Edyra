...
export const completeBatch = async (req, res, next) => {
  try {
    ...
    for (const session of activeSessions) {
      ...
      // Update and grade the corresponding submission.
      ...
      if (submission) {
        await autoSubmitExam(submission, 'admin-force', {
          reason: 'Batch completed by admin',
          ipAddress: req.ip,
          userAgent: req.headers['user-agent'],
        });
      }
...
export const forceSubmitSession = async (req, res, next) => {
  try {
    ...
    if (submission) {
      await autoSubmitExam(submission, 'admin-force', {
        reason: 'Force submitted by admin',
        ipAddress: req.ip,
        userAgent: req.headers['user-agent'],
      });
    }
  ...
  try {
    ...
    if (submission) {
      await autoSubmitExam(submission, 'auto-violation', {
        ...
        ipAddress: req.ip,
        userAgent: req.headers['user-agent'],
        totalViolations: session.violationCount || submission.totalViolations || 0,
      });
    }
