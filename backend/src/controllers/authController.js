...
export { SESSION_COOKIE_NAME, getSession, deleteSession, refreshSession, generateSessionJWT, setSessionCookie, generateAccessToken };
...
export const requestEmailVerification = async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      ...(process.env.NODE_ENV !== 'production' && { devToken: verifyToken }),
    });
  } catch (error) {
    next(error);
  }
