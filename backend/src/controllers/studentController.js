import { Exam, Question, Submission, Violation, AuditLog, Course, ExamSession } from '../models/index.js';
...
export const startExam = async (req, res, next) => {
  try {
    ...
    if (submission) {
      ...
      if (submission.isTimeUp()) {
        ...
        await ExamSession.findOneAndUpdate(
          { sessionToken: submission.sessionId },
          {
            exam: exam._id,
            ...
            browserFingerprint: req.headers['x-browser-fingerprint'] || 'lms-portal',
    ...
    // Also create/upsert ExamSession for real-time Socket.IO monitoring!
    await ExamSession.findOneAndUpdate(
  ...
  try {
    ...
    if (submission.isTimeUp()) {
      ...
      await autoSubmitExam(submission, 'auto-timeout', {
        reason: 'Time expired during answer save',
        ipAddress: req.ip,
        userAgent: req.headers['user-agent'],
      });
    ...
    const { questionId, selectedOptions, textAnswer, numericalAnswer, markedForReview, timeTaken } = req.body;
    ...
    answer.numericalAnswer = numericalAnswer ?? null;
    answer.markedForReview = markedForReview || false;
    answer.visited = true;
    answer.timeTaken = timeTaken || 0;
    if ((selectedOptions && selectedOptions.length > 0) || answer.textAnswer.trim() || answer.numericalAnswer !== null) {
...
export const bulkSaveAnswers = async (req, res, next) => {
  try {
    ...
    if (submission.isTimeUp()) {
      await autoSubmitExam(submission, 'auto-timeout', {
        reason: 'Time expired during autosave',
        ipAddress: req.ip,
        userAgent: req.headers['user-agent'],
      });
      throw new AppError('Time is up. Exam has been auto-submitted.', 400);
    ...
    for (const answerData of answers) {
      ...
      if (answer) {
        ...
        answer.textAnswer = answerData.textAnswer || '';
        answer.numericalAnswer = answerData.numericalAnswer ?? null;
        answer.markedForReview = answerData.markedForReview || false;
        answer.visited = answerData.visited ?? true;
        answer.timeTaken = answerData.timeTaken || 0;
        if (
          (answerData.selectedOptions && answerData.selectedOptions.length > 0) ||
...
export const submitExam = async (req, res, next) => {
  try {
    ...
    if (submission.isTimeUp() || req.body.autoSubmit) {
      await autoSubmitExam(submission, 'auto-timeout', {
        reason: 'Time expired',
        ipAddress: req.ip,
        userAgent: req.headers['user-agent'],
    ...
    await submission.save();
    await syncExamSessionFromSubmission(submission, 'submitted', 'submitted_from_lms', {
      ipAddress: req.ip,
    });
...
export const getDashboardData = async (req, res, next) => {
  try {
    ...
    const courses = await Course.find({ enrolledStudents: studentId })
      .select('title code department instructor status schedule')
    ...
    const recentResults = recentSubmissions
      .filter(s => s.exam)
      .map(s => ({
        id: s._id,
        examTitle: s.exam.title,
    ...
    // 4. Calculate GPA Overview
    const allEvaluated = await Submission.find({
      student: studentId,
      status: 'evaluated',
    }).select('percentage marksObtained totalMarks').lean();

    const avgPercentage = allEvaluated.length > 0 
      ? allEvaluated.reduce((acc, curr) => acc + (curr.percentage || 0), 0) / allEvaluated.length
      : 0;

    // Convert percentage to 4.0 GPA scale (rough institutional conversion)
    ...
    res.json({
      ...
      data: {
        ...
        performance: {
          ...
          totalExamsTaken: allEvaluated.length,
        }
      },
    });
  } catch (error) {
...
async function syncExamSessionFromSubmission(submission, status, action, details = {}) {
...
async function autoSubmitExam(submission, type = 'auto-timeout', options = {}) {
  ...
  const timeTaken = Math.floor((Date.now() - submission.startedAt.getTime()) / 1000);
  const statusByType = {
    'auto-timeout': 'auto-submitted',
    'auto-violation': 'violation-submitted',
    'admin-force': 'force-submitted',
  };
  const sessionStatusByType = {
    'auto-timeout': 'expired',
    'auto-violation': 'violation_terminated',
    'admin-force': 'force_submitted',
  };

  submission.status = statusByType[type] || 'auto-submitted';
  submission.submittedAt = new Date();
  submission.submissionType = ['auto-timeout', 'auto-violation', 'admin-force'].includes(type)
    ? type
    : 'auto-timeout';
  submission.terminationReason = options.reason || submission.terminationReason || null;
  submission.timeTaken = timeTaken;
  submission.submissionIp = options.ipAddress || submission.submissionIp;
  submission.submissionUserAgent = options.userAgent || submission.submissionUserAgent;
  if (options.totalViolations !== undefined) {
    submission.totalViolations = options.totalViolations;
  }

  await submission.calculateResults();
  await submission.save();
  await syncExamSessionFromSubmission(
    submission,
    sessionStatusByType[type] || 'expired',
    type === 'admin-force' ? 'force_submitted_from_submission' : 'auto_submitted_from_submission',
    {
  ...
  await AuditLog.log({
    ...
    targetType: 'submission',
    targetId: submission._id,
    details: { type, marksObtained: submission.marksObtained, reason: options.reason },
    ipAddress: options.ipAddress,
