...
examSessionSchema.methods.addViolation = async function(type, details, ipAddress) {
  ...
  if (['dev_tools', 'screen_share_detected', 'fingerprint_mismatch', 'copy_attempt', 'paste_attempt'].includes(type)) {
    anomalyWeight = 30;
  } else if (['tab_switch', 'window_blur', 'multiple_login'].includes(type)) {
    anomalyWeight = 15;
  }
  ...
  if (this.violationCount >= this.maxViolationsAllowed || this.anomalyScore >= 100) {
