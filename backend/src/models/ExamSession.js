...
const normalizeViolationType = (type = 'other') => {
  const aliases = {
    ...
    'devtools-open': 'dev_tools',
    'fullscreen-exit': 'fullscreen_exit',
    'keyboard-shortcut': 'blocked_shortcut',
    'screenshot-attempt': 'screenshot_attempt',
    'print-attempt': 'print_attempt',
    'navigation': 'navigation_attempt',
  };
...
const examSessionSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ['active', 'submitted', 'force_submitted', 'expired', 'violation_terminated', 'terminated'],
    default: 'active',
  },
  ...
  violations: [{
    type: {
      ...
      enum: [
        ...
        'desk-violation',
        'desk-unauthorized-person',
        'device-detected',
        'suspicious-device-detected',
        'head-movement-suspicious',
        'eye-gaze-mismatch',
        'room-environment-violation',
        'other'
      ],
    },
    timestamp: {
...
examSessionSchema.methods.addViolation = async function(type, details, ipAddress) {
  ...
  this.auditLog.push({
    ...
    details: { type: normalizedType, originalType: type, details },
    ipAddress,
  });

  // Calculate anomaly score based on violation type
  let anomalyWeight = 10;
  if (['dev_tools', 'screen_share_detected', 'fingerprint_mismatch', 'copy_attempt', 'paste_attempt'].includes(normalizedType)) {
    anomalyWeight = 30;
  } else if (['tab_switch', 'window_blur', 'multiple_login'].includes(normalizedType)) {
    anomalyWeight = 15;
  }
  
  this.anomalyScore += anomalyWeight;
  
  // Auto-terminate if max violations exceeded or high anomaly score
  const shouldTerminate = this.violationCount >= this.maxViolationsAllowed || this.anomalyScore >= 100;
  if (shouldTerminate) {
