...
submissionSchema.methods.getQuestionStatus = function(answer) {
  const hasChoiceAnswer = answer.selectedOptions && answer.selectedOptions.length > 0;
  const hasTextAnswer = typeof answer.textAnswer === 'string' && answer.textAnswer.trim().length > 0;
  const hasNumericalAnswer = answer.numericalAnswer !== null && answer.numericalAnswer !== undefined;
  const hasAnswer = hasChoiceAnswer || hasTextAnswer || hasNumericalAnswer;
...
submissionSchema.methods.updateAnswer = function(questionId, selectedOptions, markedForReview = false, extra = {}) {
  ...
  if (answer) {
    ...
    if (Object.prototype.hasOwnProperty.call(extra, 'textAnswer')) {
      answer.textAnswer = extra.textAnswer || '';
    }
    if (Object.prototype.hasOwnProperty.call(extra, 'numericalAnswer')) {
      answer.numericalAnswer = extra.numericalAnswer ?? null;
    }
    answer.markedForReview = markedForReview;
    answer.visited = true;
    const hasTextAnswer = typeof answer.textAnswer === 'string' && answer.textAnswer.trim().length > 0;
    const hasNumericalAnswer = answer.numericalAnswer !== null && answer.numericalAnswer !== undefined;
    answer.answeredAt = selectedOptions.length > 0 || hasTextAnswer || hasNumericalAnswer ? new Date() : null;
  }
...
submissionSchema.methods.calculateResults = async function() {
  ...
  for (const answer of this.answers) {
    ...
    if (!hasSelectedOptions && !hasTextAnswer && !hasNumericalAnswer) {
      unattempted++;
    ...
    let answerForGrading = answer.selectedOptions;
    if (['numerical'].includes(question.questionType)) {
      answerForGrading = hasNumericalAnswer ? answer.numericalAnswer : answer.textAnswer;
    } else if (['fill-blank', 'short-answer', 'long-answer', 'code'].includes(question.questionType)) {
      answerForGrading = answer.textAnswer;
    } else if (['matching', 'ordering'].includes(question.questionType)) {
      try {
        answerForGrading = hasTextAnswer ? JSON.parse(answer.textAnswer) : [];
      } catch {
        answerForGrading = [];
      }
    } else if (question.questionType === 'image-based' && !hasSelectedOptions) {
      answerForGrading = answer.textAnswer;
    }

    const marks = question.checkAnswer(answerForGrading);
