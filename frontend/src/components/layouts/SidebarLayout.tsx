...
// EDYRA LMS common sections available to all roles
const lmsCommonSections = (): NavSection[] => [
  {
    title: 'Learning',
    items: [
      { label: 'Courses', href: '/courses', icon: <Icons.List /> },
      { label: 'Assignments', href: '/assignments', icon: <Icons.FileText /> },
      { label: 'Gradebook', href: '/gradebook', icon: <Icons.Award /> },
    ],
  },
  {
    title: 'Campus',
    items: [
      { label: 'Attendance', href: '/attendance', icon: <Icons.Clock /> },
      { label: 'Communication', href: '/communication', icon: <Icons.Users /> },
    ],
  },
];

const getNavSections = (role: string): NavSection[] => {
  if (role === 'admin' || role === 'super-admin' || role === 'institute-admin') {
    return [
      {
        title: 'Main',
        items: [
      ...
      },
      {
        title: 'AI & Tools',
        items: [
          { label: 'AI Tools', href: '/ai-tools', icon: <Icons.Settings /> },
        ],
      },
      {
        title: 'Admin ERP',
        items: [
          { label: 'ERP Dashboard', href: '/erp', icon: <Icons.Dashboard /> },
        ],
      },
      {
        title: 'System',
  ...
  if (role === 'teacher' || role === 'teaching-assistant') {
    return [
      {
        title: 'Main',
        items: [
  ...
  // Student (and parent, external-evaluator fallback)
  return [
    {
      title: 'Exam Portal',
      items: [
    ...
    ...lmsCommonSections(),
    {
      title: 'AI & Tools',
      items: [
        { label: 'AI Study Tools', href: '/ai-tools', icon: <Icons.Settings /> },
        { label: 'Analytics', href: '/analytics', icon: <Icons.BarChart /> },
      ],
    },
    {
      ...
      <aside className={`inst-sidebar ${collapsed ? 'collapsed' : ''}`}>
        <div className="inst-sidebar-header">
          <div className="inst-sidebar-logo">
            <span className="inst-logo-icon">ED</span>
            {!collapsed && <span className="inst-logo-text">EDYRA</span>}
          </div>
        </div>
        ...
        <div className="inst-sidebar-footer">
          ...
          {!collapsed && (
            <div className="inst-sidebar-version">&copy; 2026 EDYRA Academic LMS</div>
          )}
        </div>
      </aside>
