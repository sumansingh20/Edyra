...
export default function AdminProfilePage() {
  ...
  return (
    <LMSLayout pageTitle="Administrator Profile" breadcrumbs={[{ label: 'Dashboard', href: '/admin/dashboard' }, { label: 'Profile' }]}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 24 }}>
        <div className="lms-main-col">
          <div className="lms-section" style={{ textAlign: 'center', padding: 40 }}>
            <div style={{ width: 120, height: 120, borderRadius: '50%', background: 'var(--danger)', color: '#fff', fontSize: 48, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
              {user?.firstName?.[0]}{user?.lastName?.[0]}
            </div>
            <h2 style={{ margin: '0 0 4px' }}>{user?.firstName} {user?.lastName}</h2>
            <div style={{ color: 'var(--text-muted)', fontSize: 14 }}>System Administrator</div>
            <div style={{ marginTop: 20 }}>
              <span className="lms-status lms-status-active">Super Admin Status</span>
            </div>
          </div>
        </div>
