...
// Map frontend-detected events to backend Joi `violationSchemas.report.type`
const violationTypeMap: Record<ViolationEvent, string> = {
  'tab-switch': 'tab-switch',
  'copy-paste': 'copy-attempt', // further refined by the calling handler
...
const normalizeQuestion = (rawQuestion: any): Question => {
  const raw = rawQuestion?.question || rawQuestion || {};
  const questionId = String(rawQuestion?.questionId || rawQuestion?.id || rawQuestion?._id || raw?.id || raw?._id || '');
  ...
  return {
    ...
    markedForReview: !!rawQuestion?.markedForReview,
    selectedOptions: (rawQuestion?.selectedOptions || []).map((id: any) => String(id)),
    numericalAnswer: rawQuestion?.numericalAnswer,
    textAnswer: rawQuestion?.textAnswer || '',
    isVisited: !!rawQuestion?.visited,
  };
};
...
export default function StudentExamPage() {
  ...
  // Submission ID (backend routes: /student/submissions/:id/*)
  ...
  const lastViolationTypeRef = useRef<ViolationEvent | null>(null);

  const logViolation = useCallback(async (type: ViolationEvent, angle?: 'primary' | 'secondary') => {
    if (!submissionIdRef.current) return;

    // Throttle: avoid spamming API on keydown/copy/paste handlers
    const now = Date.now();
    const lastAt = lastViolationAtRef.current;
    const lastType = lastViolationTypeRef.current;
    const sameType = lastType === type;

    if (sameType && now - lastAt < 1200) return;
    ...
    const backendType = violationTypeMap[type];
    const details = `Violation detected: ${type}`;

    if (socketRef.current?.connected) {
      // Socket handler tolerates other fields, but keep payload aligned to backend naming
      socketRef.current.emit('violation', { type: backendType, details, angle });
      return;
    }


    try {
      await api.post(`/student/submissions/${submissionIdRef.current}/violation`, {
        type: backendType,
        description: angle ? `[${angle}] ${details}` : details,
      });
    } catch {
      /* silent — still count locally */
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const handleVisibility = () => {
      if (document.hidden) {
        logViolation('tab-switch', 'primary');
        setWarningMessage('️ Security Violation: Tab switch/minimize detected.');
    ...
    const handleFullscreenChange = () => {
      ...
      if (!full) logViolation('fullscreen-exit', 'primary');
    };
    const handleContextMenu = (e: MouseEvent) => { e.preventDefault(); logViolation('context-menu', 'primary'); };
    const handleCopy = (e: ClipboardEvent) => { e.preventDefault(); logViolation('copy-paste', 'primary'); };
    const handlePaste = (e: ClipboardEvent) => { e.preventDefault(); logViolation('copy-paste', 'primary'); };
    const handlePrint = () => { logViolation('copy-paste', 'primary'); };
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (blocked) { e.preventDefault(); logViolation('copy-paste', 'primary'); }
    };
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const isVerified = sessionStorage.getItem(`exam_verified_${examId}`);
        if (!isVerified) {
          router.replace(`/student/exam/${examId}/secure-entry`);
          return;
        }
  ...
  useEffect(() => {
    ...
    autoSaveIntervalRef.current = setInterval(() => {
      if (submissionIdRef.current && Object.keys(answers).length > 0) {
        ...
        api.post(`/student/submissions/${submissionIdRef.current}/answers`, { answers: bulk }).catch(() => {});
  ...
  const startExam = async () => {
    try {
      ...
      // Backend returns: data.submission + data.state
      // Submission routes expect submission._id
      // Backend `startExam` returns: data.submission = formatSubmissionResponse(submission)
      // where `id` is the Submission document id.
      submissionIdRef.current = data?.submission?.id || '';
      sessionTokenRef.current = data?.submission?.sessionId || '';

      if (!submissionIdRef.current) {
        throw new Error('Submission id missing from exam start response');
      }

      // Questions are returned as `data.questions`
      const normalizedQuestions = (data.questions || []).map(normalizeQuestion).filter((q: Question) => q.questionId);
      const remainingTime = data?.state?.remainingTime || data?.submission?.remainingTime || (examData?.duration || 0) * 60;

      setSession({
        _id: submissionIdRef.current,
        exam: {
          _id: String(examId),
          title: data?.examTitle || examData?.title || 'Examination',
          ...
          negativeMarkingValue: examData?.negativeMarkingValue || 0,
          allowReview: examData?.allowReview || false,
        },
        questions: normalizedQuestions,
        status: data?.submission?.status || 'in-progress',
        remainingTime,
        currentQuestion: data?.state?.currentQuestion || 0,
      });

      const initialAnswers: typeof answers = {};
      normalizedQuestions.forEach((q: Question) => {
        // Backend answer keys are questionId-like. Use backend question._id string.
        const qId = q.questionId;
        if (!qId) return;
        initialAnswers[qId] = {
          selected: q.selectedOptions || [],
          numerical: q.numericalAnswer,
          text: q.textAnswer,
          flagged: !!q.markedForReview,
        };
      });
  ...
  const handleSubmit = async (auto = false) => {
    ...
    try {
      await api.post(`/student/submissions/${submissionIdRef.current}/submit`, { autoSubmit: auto });
  ...
  const selectOption = (questionId: string, optionId: string, type: string) => {
    const cur = answers[questionId] || { selected: [], flagged: false };
    const selected = type === 'mcq'
      ? (cur.selected.includes(optionId) ? [] : [optionId])
      : (cur.selected.includes(optionId) ? cur.selected.filter(o => o !== optionId) : [...cur.selected, optionId]);

    setAnswers(prev => ({ ...prev, [questionId]: { ...(prev[questionId] || cur), selected } }));

    if (socketRef.current?.connected) {
      socketRef.current.emit('save-answer', { questionId, answer: { selectedOptions: selected } });
    }
    // Fire-and-forget save
    if (submissionIdRef.current) {
      api.post(`/student/submissions/${submissionIdRef.current}/answer`, {
        questionId,
  ...
  const updateWrittenAnswer = (questionId: string, patch: { numerical?: number; text?: string }) => {
    const nextAnswer = {
      ...(answers[questionId] || { selected: [], flagged: false }),
      ...patch,
    };
    ...
    saveRef.current = setTimeout(() => {
      ...
      api.post(`/student/submissions/${submissionIdRef.current}/answer`, {
        questionId,
        selectedOptions: nextAnswer.selected || [],
        numericalAnswer: nextAnswer.numerical,
        textAnswer: nextAnswer.text || '',
      }).catch(() => {});
    }, 600);
  };
  ...
  const flagQuestion = (questionId: string) => {
    ...
    if (submissionIdRef.current) {
      api.post(`/student/submissions/${submissionIdRef.current}/visit`, { questionId, markedForReview: !answers[questionId]?.flagged }).catch(() => {});
    }
  };
            ...
            <div>
              ...
              {currentQ.question?.questionType === 'numerical' && (
                <input type="number" style={{ width: '100%', padding: 14, fontSize: 18, borderRadius: 8, border: '2px solid #dee2e6', fontFamily: 'monospace' }}
                  ...
                  onChange={e => updateWrittenAnswer(currentQ.questionId, { numerical: e.target.value === '' ? undefined : parseFloat(e.target.value) })} />
              )}
              ...
              {currentQ.question?.questionType === 'descriptive' && (
                <textarea style={{ width: '100%', padding: 14, fontSize: 14, borderRadius: 8, border: '2px solid #dee2e6', minHeight: 160, resize: 'vertical' }}
                  ...
                  onChange={e => updateWrittenAnswer(currentQ.questionId, { text: e.target.value })} />
              )}
