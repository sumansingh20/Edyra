...
export default function ExamMonitorPage() {
  ...
  // Socket.IO real-time monitor connection
  useEffect(() => {
    if (!isAuthenticated || !user || (user.role !== 'admin' && user.role !== 'teacher')) return;

    const socketUrl = process.env.NEXT_PUBLIC_API_URL?.replace('/api', '') || window.location.origin;
    const socket = io(`${socketUrl}/exam-monitor`, {
    ...
    socket.on('connect', () => {
      console.log('[Socket] Connected to /exam-monitor namespace');
      socket.emit('join-exam-monitor', { examId, token: 'teacher-token' });
    });

    socket.on('exam-stats', (stats: any) => {
      console.log('[Socket] Live exam stats update:', stats);
    });

    socket.on('violation-alert', (data: any) => {
      console.warn('[Socket] Live violation alert:', data);
      toast.error(`⚠️ Violation: ${data.studentName || 'Student'} (${data.type})`);
      setSessions(prev => prev.map(s => (s._id === data.sessionId || s.student._id === data.studentId) ? { ...s, violationCount: (s.violationCount || 0) + 1, lastActivityAt: new Date().toISOString() } : s));
      setViolations(prev => [{ _id: String(Date.now()), student: { firstName: data.studentName || 'Student', lastName: '' }, type: data.type, severity: data.severity || 'medium', createdAt: new Date().toISOString() }, ...prev].slice(0, 20));
    });

    socket.on('student-submitted', (data: any) => {
      console.log('[Socket] Student submitted:', data);
      toast.success(`✅ Submitted: ${data.studentName || 'Student'}`);
      setSessions(prev => prev.map(s => (s._id === data.sessionId || s.student._id === data.studentId) ? { ...s, status: 'submitted', submittedAt: new Date().toISOString(), lastActivityAt: new Date().toISOString() } : s));
    });

    socket.on('session-terminated-alert', (data: any) => {
      console.warn('[Socket] Session terminated alert:', data);
      toast.error(`❌ Terminated: ${data.studentName || 'Student'} (${data.reason})`);
      setSessions(prev => prev.map(s => (s._id === data.sessionId || s.student._id === data.studentId) ? { ...s, status: 'violation_terminated', lastActivityAt: new Date().toISOString() } : s));
    });
  ...
  const handleBroadcast = () => {
    ...
    if (socketRef.current?.connected) {
      socketRef.current.emit('broadcast-message', { examId, message: broadcastMessage });
      toast.success('📢 Announcement broadcasted to all students!');
      setBroadcastMessage('');
    } else {
      toast.error('Socket not connected. Please check connection.');
    }
  };
  ...
  const handleForceSubmit = async (sessionId: string) => {
    ...
    if (socketRef.current?.connected) {
      socketRef.current.emit('force-submit-student', { sessionId, examId });
    }
    try {
      const basePath = user?.role === 'admin' ? '/admin' : '/teacher';
  ...
  const handleTerminate = async (sessionId: string) => {
    ...
    setActionLoading(true);
    if (socketRef.current?.connected) {
      socketRef.current.emit('terminate-session', { sessionId, examId });
    }
    try {
      const basePath = user?.role === 'admin' ? '/admin' : '/teacher';
        ...
        <div className="p-6">
          ...
          <div className="bg-white rounded-lg shadow p-6 mb-6 border-l-4 border-blue-600">
            <h2 className="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
              ...
              <span className="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded">Socket.IO Real-Time</span>
            </h2>
            <p className="text-sm text-gray-600 mb-4">
              Instantly push a live alert banner to all active student screens taking this examination.
            </p>
            <div className="flex gap-4">
              <input
                ...
                placeholder="Type your announcement here (e.g., '10 minutes remaining', 'Correction in Q4')..."
                className="flex-1 border border-gray-300 rounded px-4 py-2 text-sm focus:outline-none focus:border-blue-600"
                onKeyDown={(e) => { if (e.key === 'Enter') handleBroadcast(); }}
              />
              <button
                onClick={handleBroadcast}
                className="bg-blue-900 text-white px-6 py-2 rounded text-sm font-semibold hover:bg-blue-800 flex items-center gap-2 shadow"
              >
