...
export default function AssignmentDetailPage() {
  ...
  if (loading) return <LMSLayout><div className="text-center py-12 text-gray-500">Loading...</div></LMSLayout>;
  if (!assignment) return <LMSLayout><div className="text-center py-12 text-gray-500">Not found</div></LMSLayout>;

  const isPastDue = new Date(assignment.dueDate) < new Date();
  const mySubmission = assignment.mySubmission;

  return (
    <LMSLayout breadcrumbs={[{ label: 'Edyra' }, { label: 'Assignments', href: '/assignments' }, { label: assignment.title }]}>
