...
export default function ExamAttemptPage() {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      socket.emit('join-exam', { sessionToken, fingerprint });
    });
  ...
  }, [sessionToken, fingerprint, examId, API_URL]);
  ...
  useEffect(() => {
    ...
    const getFingerprint = async () => {
      ...
      try {
        ...
        const timeoutPromise = new Promise<null>((_, reject) => setTimeout(() => reject(new Error('Fingerprint timeout')), 1000));
  ...
  const reportViolation = async (type: string, details: string) => {
    ...
    if (socketRef.current?.connected) {
      socketRef.current.emit('violation', { type, details });
    }
  ...
  const saveAnswer = async (questionId: string, answer: Answer) => {
    ...
    if (socketRef.current?.connected) {
      socketRef.current.emit('save-answer', { questionId, answer });
    }
